...
def load_admin_data():
    try:
        ...
        for b in books:
            def clean_str(val): return str(val) if pd.notna(val) and str(val).lower() != 'nan' else ''
            ...
            for key in ['ลิงก์อ่าน', 'ลิงก์ต้นฉบับ']:
                ...
                if not isinstance(b[key], list): b[key] = []
...
if 'main_menu' not in st.session_state: st.session_state.main_menu = "📊 สรุปภาพรวม"
menu = st.sidebar.radio("ระบบนำทาง", menu_options, key="main_menu")
if menu != "📚 จัดการนิยาย & ไฟล์": st.session_state.selected_book_idx = None
...
if menu == "📊 สรุปภาพรวม":
    ...
    if st.button("🔄 โหลดข้อมูลใหม่ (Clear Cache)", type="primary"): st.cache_data.clear(); st.rerun()
    ...
    with col4: st.markdown(f"<div class='metric-card'><h3>💰 ยอดสุทธิรวม</h3><h2 style='color:#6C63FF;'>฿{total_revenue:,.0f}</h2></div>", unsafe_allow_html=True)
...
# ------------------------------------------
    ...
    if st.session_state.selected_book_idx is not None:
        ...
        if st.button("🔙 กลับหน้าหลัก"): st.session_state.selected_book_idx = None; st.rerun()
        ...
        with c_img: 
            ...
            if uploaded_file and st.button("🚀 ยืนยันอัปโหลด", use_container_width=True):
                ...
                if new_url: st.session_state.books_data[idx]['ภาพปก'] = new_url; save_all(); st.rerun()
...
# ------------------------------------------
    ...
    with tab1:
        ...
        for idx, b in enumerate(st.session_state.books_data):
            ...
            with st.expander(f"📖 {b['ชื่อเรื่อง']} | ตอนที่แปล: {b.get('ตอนปัจจุบัน', 0)} / {b.get('เป้าหมาย', 1)}"):
                ...
                with c1: safe_image(b.get('ภาพปก'), "rank-img")
                with c2:
                    ...
                    if c_btn.button("💾 บันทึกยอด", key=f"sv_{idx}", type="primary"):
                        ...
                        st.session_state.books_data[idx]['ตอนปัจจุบัน'] = new_chap; save_all(); st.rerun()
    ...
    with tab2:
        ...
        if not df_prog.empty:
            ...
            with c1: st.markdown(f"<div class='progress-box'><h4>💖 ตอง (เดือนนี้)</h4><div class='progress-value val-tong'>{int(tong_m)}</div></div>", unsafe_allow_html=True)
            with c2: st.markdown(f"<div class='progress-box'><h4>💙 ตาว (เดือนนี้)</h4><div class='progress-value val-tao'>{int(tao_m)}</div></div>", unsafe_allow_html=True)
            with c3: st.markdown(f"<div class='progress-box'><h4>🌍 รวมทีม (เดือนนี้)</h4><div class='progress-value' style='color:#6C63FF;'>{int(tong_m + tao_m)}</div></div>", unsafe_allow_html=True)
        ...
        if qc_books:
            ...
            if st.button("💾 บันทึกยอดรายรับทั้งหมด", type="primary"):
                ...
                if new_entries:
                    ...
                    save_all(); st.rerun()
                else: st.warning("ไม่มียอดให้บันทึกครับ")
        else: st.warning("ไม่พบนิยายของ QC ท่านนี้")

    with tab2:
        edited_finance = st.data_editor(st.session_state.finance_db, num_rows="dynamic", use_container_width=True)
        if st.button("💾 บันทึกตารางฐานข้อมูล"): st.session_state.finance_db = edited_finance; save_all(); st.rerun()
...
# ------------------------------------------
    ...
    c1, c2 = st.columns(2)
    with c1: st.subheader("📚 หมวดหมู่นิยาย"); ed_c = st.data_editor(pd.DataFrame(st.session_state.app_settings['categories'], columns=['ชื่อหมวดหมู่']), num_rows="dynamic", use_container_width=True)
    with c2: st.subheader("🌐 แพลตฟอร์มเผยแพร่"); ed_p = st.data_editor(pd.DataFrame(st.session_state.app_settings['platforms'], columns=['ชื่อแพลตฟอร์ม']), num_rows="dynamic", use_container_width=True)
